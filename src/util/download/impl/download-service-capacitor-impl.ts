...
import { DownloadManager } from 'capacitor-plugin-downloadmanager';
...
/**
 * Callback-style adapter over the promise-based `capacitor-plugin-downloadmanager`,
 * matching the ambient `downloadManager` global (declared in
 * `plugins/cordova-plugin-android-downloadmanager.d.ts`) that this class and several
 * other services (content/course/certificate services, telemetry auto-sync) depend on.
 * Installing this on `window['downloadManager']` lets all of those keep calling the
 * same global, unchanged, on the Capacitor platform.
 */
function createDownloadManagerShim() {
    return {
        enqueue(request: EnqueueRequest, callback?: (err, id: string) => void) {
            const { uri, headers, ...rest } = request;
            DownloadManager.enqueue({ ...rest, url: uri, headers: [] })
                .then((result) => callback && callback(undefined, result.id))
                .catch((err) => callback && callback(err, undefined as unknown as string));
        },
        query(filter: EnqueueFilter | undefined, callback?: (err, entries: EnqueuedEntry[]) => void) {
            DownloadManager.query(filter)
                .then((result) => callback && callback(undefined, result.downloads as EnqueuedEntry[]))
                .catch((err) => callback && callback(err, []));
        },
        remove(ids: string[], callback?: (err, removeCount: number) => void) {
            DownloadManager.remove({ ids })
                .then((result) => callback && callback(undefined, result.removed))
                .catch((err) => callback && callback(err, 0));
        },
        fetchSpeedLog(successCallback: (args: any, downloadSpeedLog: DownloadSpeedLog) => void, errorCallback: () => void) {
            DownloadManager.fetchSpeedLog()
                .then((result) => successCallback(undefined, result))
                .catch(() => errorCallback());
        }
    };
}
...
@injectable()
export class DownloadServiceCapacitorImpl implements DownloadService, SdkServiceOnInitDelegate, ContentDeleteListener {
    ...
    constructor(@inject(InjectionTokens.EVENTS_BUS_SERVICE) private eventsBusService: EventsBusService,
        ...
        window['downloadManager'] = createDownloadManagerShim();

        this.sharedPreferencesSetCollection = new SharedPreferencesSetCollectionImpl(
            this.sharedPreferences,
            DownloadServiceCapacitorImpl.KEY_TO_DOWNLOAD_LIST,
        ...
        return this.sharedPreferencesSetCollection.asSet()
            .pipe(
                mergeMap((downloadListAsSet: Set<DownloadRequest>) => {
                    ...
                    // Temporary diagnostic — remove once resume-after-cancel is confirmed
                    // picking the right item (shows the FULL persisted queue, so a stale
                    // leftover entry from before a cancel is distinguishable from a fresh
                    // re-enqueue).
                    console.log('[switchToNextDownloadRequest] full persisted queue:',
                        downloadListAsSet.toArray().map(r => ({ id: r.identifier, priority: r.withPriority || 0 })));
                    ...
                    console.log('[switchToNextDownloadRequest] picked:', anyDownloadRequest.identifier);
                    ...
                    return new Observable<string>((observer) => {
                        downloadManager.enqueue({
                            ...
                            destinationInExternalFilesDir: {
                                dirType: DownloadServiceCapacitorImpl.DOWNLOAD_DIR_NAME,
                        ...
                        tap(async (downloadId) => {
                            ...
                            anyDownloadRequest.downloadedFilePath = dataDirectory +
                                DownloadServiceCapacitorImpl.DOWNLOAD_DIR_NAME + '/' + anyDownloadRequest.filename;
                            anyDownloadRequest.downloadId = downloadId;
                            this.currentDownloadRequest$.next(anyDownloadRequest);
                        }),
                        tap(async () => await DownloadServiceCapacitorImpl.generateDownloadStartTelemetry(anyDownloadRequest!)),
                        mapTo(undefined),
                        catchError(() => {
                            return this.cancel({
                    ...
                    return this.sharedPreferencesSetCollection.remove(toRemoveDownloadRequest)
                        .pipe(
                            ...
                            tap(async () => generateTelemetry
                                && await DownloadServiceCapacitorImpl.generateDownloadCancelTelemetry(toRemoveDownloadRequest))
                        );
                })
            );
    }
                                ...
                                return from(downloadRequests)
                                    .pipe(
                                        tap(async (downloadRequest) =>
                                            await DownloadServiceCapacitorImpl.generateDownloadCancelTelemetry(downloadRequest)),
                ...
                mergeMap((currentDownloadRequest) => {
                    if (downloadProgress.payload.status === DownloadStatus.STATUS_SUCCESSFUL) {
                        ...
                        return iif(
                            ...
                            defer(async () => {
                                DownloadServiceCapacitorImpl.generateDownloadCompleteTelemetry(currentDownloadRequest!);
                                // Must be awaited: onDownloadCompletion() runs the full import and
                                // then calls cancel() to advance currentDownloadRequest$ to the next
                                // queued item. Firing it without awaiting let this observable
                                // complete immediately, so a download() call arriving in that window
                                // still saw the just-finished item as "current" and only appended to
                                // the queue instead of switching — leaving currentDownloadRequest$
                                // (and therefore the progress UI) stuck on the stale item.
    ...
    private getDownloadProgress(downloadRequest: DownloadRequest): Observable<DownloadProgress> {
        return new Observable((observer) => {
            downloadManager.query({ ids: [downloadRequest.downloadId!] }, (err, entries) => {
                ...
                if (!entry) {
                    // The native DownloadManager row is gone (e.g. removed by a cancel that raced
                    // with this poll tick) rather than erroring — treat it as failed instead of
                    // throwing on undefined property access, which would otherwise surface as an
                    // unhandled rejection inside this async plugin callback.
                    observer.next({
                        type: DownloadEventType.PROGRESS,
                        payload: {
                            downloadId: downloadRequest.downloadId,
